...
class visualizationClass(ModelPerformance):
    ...
    def __init__(
        self, X_test, labels, XPER_values, XPER_v, XPER_v_ind, benchmark_v_ind, p, X
    ):
        ...
        self.benchmark_v_ind = pd.DataFrame(
            XPER_values[1][:, 0], columns=["Individual Benchmark"]
        )
    ...
    def bar_plot(XPER_values, p, X_test, labels, percentage=True):
        ...
        selection = pd.DataFrame(
            Contribution[1:], index=X_test.columns, columns=["Metric"]
        )
        ...
        fig.update_layout(
            yaxis=dict(autorange="reversed"), showlegend=False, plot_bgcolor="white"
        )
    ...
    def beeswarn_plot(XPER_values, X_test, labels):
        ...
        shap_values.base_values = np.reshape(
            benchmark_v_ind.to_numpy(), benchmark_v_ind.shape[0]
        )
    ...
    def force_plot(
        XPER_values, instance, X_test, variable_name, figsize=(8, 4), min_perc=0.00001
    ):
        ...
        X = pd.Series(np.round(X_test.iloc[ind_i, :], 3), index=variable_name).astype(
            object
        )
        ...
        index_sort_pos = np.argsort(
            -XPER[XPER > 0]
        )  # signe - car on index ensuite par les variables les plus importantes

        index_sort_neg = np.argsort(
            XPER[XPER < 0]
        )  # signe - car ordonne par ordre croissant
        ...
        fig, ax = visualizationClass.draw_labels(
            fig=fig,
            ax=ax,
            ...
            X_names=X_names[XPER > 0][index_sort_pos],
        ...
        fig, ax = visualizationClass.draw_labels(
            ...
            X=X[XPER < 0][index_sort_neg],
            X_names=X_names[XPER < 0][index_sort_neg],
            offset_text=offset_text,
            min_perc=min_perc,
            text_rotation=0,
        )
        ...
        # Texte pour montrer individual payoff
        text_out_val_PM = plt.text(
            perf_value,
            0.38,
            "Performance",  # hauteur de 0.33
            fontsize=14,
            alpha=0.5,
            horizontalalignment="center",
        )
        ...
        text_out_val_PM = plt.text(
            perf_value,
            0.29,
            "{}".format(perf_value),  # hauteur de 0.33
            fontsize=16,
            horizontalalignment="center",
            weight="bold",
        )
        ...
        visualizationClass.draw_higher_lower_element(
            out_value=perf_value, offset_text=offset_text
        )
    ...
    def draw_labels(
        ...
        feature_contribution = features / np.sum(
            features
        )  #      np.abs(features/perf_value)
        ...
        for i, feature in enumerate(features):
            ...
            if (sign * box_end_) > (sign * val):
                ...
                x, y = np.array([[val, box_end, box_end], [0.03, -0.08, -0.18]])
                line = lines.Line2D(x, y, lw=1.0, alpha=0.5, color=colors[0])
                line.set_clip_on(False)
                ax.add_line(line)
                start_text = box_end
        ...
        if feature_type == "negative":
            colors = (
                np.array([(30, 136, 229), (255, 255, 255)]) / 255.0
            )  # np.array([(255, 13, 87), (255, 255, 255)]) / 255.
        else:
            colors = (
                np.array([(255, 13, 87), (255, 255, 255)]) / 255.0
            )  # np.array([(30, 136, 229), (255, 255, 255)]) / 255.
