...
def XPER_choice(
    ...
    execution_type: Choose between "ThreadPoolExecutor" and "ProcessPoolExecutor"
    """
    ...
    if (getattr(model, "predict_proba", "No") != "No") and (
        getattr(model, "predict", "No") != "No"
    ):
        ...
        if Eval_Metric[0] in ["AUC", "BS"]:  # Use "predict_proba"
            if (
                intercept == True
            ):  # If an intercept is on the first column of the database
                # used to estimate the model
                X = np.delete(
                    X, 0, axis=1
                )  # delete the first column (intercept) from the
    ...
    elif (getattr(model, "predict_proba", "No") == "No") and (
        getattr(model, "predict", "No") == "No"
    ):
        print(
    ...
    liste = [
        x + 1 for x in liste
    ]  # To move index from (0, p-1) to (1, p) / list from 1 to p
    ...
    if kernel == True:
        combination_list_sampled = sampled_coalitions(
            N_coalition_sampled=N_coalition_sampled, nb_var=p
        )
    else:
        variable = liste.copy()  # Copy the list with numbers from 1 to p
        variable.remove(
            var_interet + 1
        )  # Remove the variable of interest from "variable"
        # List of every possible coalitions
        combination_list = [
            list(combinations(variable, combination))
            for combination in range(len(variable) + 1)
        ]
        combination_list = list(chain.from_iterable(combination_list))
        combination_list_sampled = random.sample(
            combination_list, N_coalition_sampled
        )  # sample without replacement (no need in reality)

    weight = np.zeros(
        shape=(N_coalition_sampled, 1)
    )  # column vector with N_coalition_sampled 0 elements
    Metric = np.zeros(
        shape=(N_coalition_sampled)
    )  # vector with N_coalition_sampled 0 elements
    Metric_ind = np.zeros(
        shape=(N_coalition_sampled, len(y))
    )  # Matrix N_coalition_sampled x len(y) filled with 0

    if kernel != True:
        Benchmark_ind = np.zeros(
            shape=(N_coalition_sampled, len(y))
        )  # Matrix N_coalition_sampled x len(y) filled with 0
        EM_ind = np.zeros(
            shape=(N_coalition_sampled, len(y))
        )  # Matrix N_coalition_sampled x len(y) filled with 0
        Metric_vinteret = np.zeros(
            shape=(N_coalition_sampled)
        )  # vector with N_coalition_sampled 0 elements
        Metric_ind_vinteret = np.zeros(
            shape=(N_coalition_sampled, len(y))
        )  # Matrix N_coalition_sampled x len(y) filled with 0
        benchmark = np.zeros(
            shape=(N_coalition_sampled)
    ...
    executor_class = (
        ThreadPoolExecutor
        if execution_type == "ThreadPoolExecutor"
        else ProcessPoolExecutor
    )

    with executor_class(max_workers=60) as executor:
        s_batches = list(
            OptimizationClass.make_batches(range(N_coalition_sampled), chunk_size)
        )
        ...
        if kernel == True:
            results = [
                executor.submit(
                    ...
                    Metric_vinteret=None,
                    Metric_ind_vinteret=None,
                    var_interet=None,
                    CFP=CFP,
                    CFN=CFN,
                    kernel=True,
        ...
        with tqdm(
            total=N_coalition_sampled,
            desc=(
                f"Coalitions"
                if kernel == True
                else f"Coalitions for variable {var_interet+1}"
            ),
            unit="coalition",
            leave=True,
            position=0,
