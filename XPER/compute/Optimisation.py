...
class OptimizationClass:
    ...
    def loop_choice(
        ...
        if kernel == True:
            ...
            weight_s = (p - 1) / (
                (
                    math.factorial(p)
                    / (math.factorial(len(S)) * math.factorial(p - len(S)))
                )
                * (len(S))
                * (p - len(S))
            )
            ...
            weight_s = (
                (math.factorial(len(S))) * (math.factorial(p - len(S) - 1))
            ) / math.factorial(p)
        ...
        for j in range(len(y)):  # Loop for each individual
            ...
            X_tirage_i[:, S] = X[
                j, S
            ].copy()  # Change the values of feature S for all individuals
            ...
            if Eval_Metric == ["AUC"]:
                ...
                X_shuffle_combination[:, S] = X[
                    :, S
                ].copy()  # Change the value of the feature
                ...
                G = (y[j] * delta_n1 + (1 - y[j]) * delta_n2) / globals()[
                    "delta_1"
                ]  # delta_1 created with globals()['delta_{}'.format(d)] // different from delta_n1
                ...
                G = 0.5 * (
                    (y_hat_pred_i * y[j]) / globals()["delta_1"]
                    + ((1 - y_hat_pred_i) * (1 - y[j]) / globals()["delta_2"])
                )
                ...
                y_hat_pred_i = model_predict(X_tirage_i, model)
                G = -(
                    CFP * ((y_hat_pred_i == 0) * (y[j] == 1))
                    + CFN * ((y_hat_pred_i == 1) * (y[j] == 0))
                )
            ...
            if kernel != True:
                ...
                if Eval_Metric == ["AUC"]:
                    ...
                    X_shuffle_combination_vinteret[:, var_interet] = X[
                        :, var_interet
                    ].copy()
                    # Change the value of the feature of interest in the shuffled database
                    # to the one of individual i

                    y_hat_tirage_vinteret = model_predict(
                        X_shuffle_combination_vinteret, model
                    )
                    # N predictions / for all individuals

                    y_hat_proba_i_vinteret = model_predict(
                        X_tirage_i_vinteret, model
                    )  # N predictions
                    # N predictions / Individual i feature values in subset S

                    y_hat_proba_i_vinteret = pd.DataFrame(
                        y_hat_proba_i_vinteret, columns=["proba"]
                    )
                    # Put the predictions made from Individual i feature values in subset S in
                    # a DataFrame / Column name "proba"
                    ...
                    y_hat_tirage_vinteret = pd.DataFrame(
                        np.repeat(y_hat_tirage_vinteret.values, N, axis=0),
                        columns=["tirage"],
                    )
                    ...
                    y_temp_vinteret = pd.DataFrame(
                        {"y": [y]}
                    )  # DataFrame with target values / column name "y"

                    y_temp_vinteret = pd.DataFrame(
                        np.repeat(y_temp_vinteret.values, N, axis=0), columns=["y"]
                    )
                    ...
                    df_temp_vinteret = pd.concat(
                        [
                            y_hat_proba_i_vinteret,
                            y_hat_tirage_vinteret,
                            y_temp_vinteret,
                        ],
                        axis=1,
                    )
                    ...
                    G_vinteret = (
                        y[j] * delta_n1_vinteret + (1 - y[j]) * delta_n2_vinteret
                    ) / globals()[
                        "delta_1"
                    ]  # delta_1 created with globals()['delta_{}'.format(d)] // different from delta_n1
                    # Compute the individual i contribution to the performance metric
                    ...
                    G_vinteret = 0.5 * (
                        (y_hat_pred_i_vinteret * y[j]) / globals()["delta_1"]
                        + (
                            (1 - y_hat_pred_i_vinteret)
                            * (1 - y[j])
                            / globals()["delta_2"]
                        )
                    )
                    ...
                    y_hat_pred_i_vinteret = model_predict(X_tirage_i_vinteret, model)
                    G_vinteret = (y_hat_pred_i_vinteret * y[j]) + (
                        (1 - y_hat_pred_i_vinteret) * (1 - y[j])
                    )
                    ...
                    y_hat_pred_i_vinteret = model_predict(X_tirage_i_vinteret, model)
                    G_vinteret = ((1 - y_hat_pred_i_vinteret) * (1 - y[j])) / globals()[
                        "delta_1"
                    ]
                    ...
                    y_hat_pred_i_vinteret = model_predict(X_tirage_i_vinteret, model)
                    G_vinteret = -(
                        CFP * ((y_hat_pred_i_vinteret == 0) * (y[j] == 1))
                        + CFN * ((y_hat_pred_i_vinteret == 1) * (y[j] == 0))
                    )
                    ...
                    y_hat_pred_i_vinteret = model_predict(X_tirage_i_vinteret, model)
                    G_vinteret = (
                        1 - ((y[j] - y_hat_pred_i_vinteret) ** 2) / globals()["delta_1"]
                    )
        ...
        if kernel != True:
            Metric_s_vinteret = np.sum(G_i_j_vinteret) / (
                (N) ** 2
            )  # np.mean(Metric_ind_j_vinteret)
    ...
    def loop_choice_batch(
        ...
        for s in s_batch:
            ...
            res = OptimizationClass.loop_choice(
                s,
                combination_list_sampled,
                p,
                X,
