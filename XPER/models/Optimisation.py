...
class OptimizationClass:
    ...
    def loop_AUC_pickle(self, s, combination_list_sampled, p, X, y, Pred_Formula, delta_n3, Metric, Metric_ind, N, X_shuffle, Metric_vinteret, Metric_ind_vinteret, var_interet):
        '''
        s : number of the iteration (coalition)
        combination_list_sampled : combination of features in which to compute the marginal effect on the AUC
        p : number of features
        X : feature values
        y : target values
        Pred_Formula : estimated model
        delta_n3 : denominator of the AUC
        Metric : AUC(s) computed without the variable of interest for each combination of feature (filled as it goes along the loop)
        Metric_ind ; Individual AUC computed without the variable of interest for each combination of feature (filled as it goes along the loop)
        N : Sample size
        X_shuffle : Feature values shuffled
        Metric_vinteret : AUC(s) computed with the variable of interest for each combination of feature (filled as it goes along the loop)
        Metric_ind_vinteret : Individual AUC computed with the variable of interest for each combination of feature (filled as it goes along the loop)
        ...
        weight_s = ((math.factorial(len(S)))*(math.factorial(p-len(S)-1)))/math.factorial(p) 
            # Weight associated with the coalition

        # Code specific to the measure            
        X_shuffle_combination = X_shuffle.copy()      
        X_shuffle_combination[:,S] = X[:,S].copy()   # Change the value of the feature values shuffled in column S 
    ...
    def loop_AUC_Kernel(self, s, combination_list_sampled, p, X, y, Pred_Formula, delta_n3, Metric, Metric_ind, N, X_shuffle):
        '''
        s : number of the iteration (coalition)
        combination_list_sampled : combination of features in which to compute the marginal effect on the AUC
        p : number of features
    ...
    def loop_AUC(self, s, combination_list_sampled, p, X, y, Pred_Formula, delta_n3, Metric, Metric_ind, N, X_shuffle, Metric_vinteret, Metric_ind_vinteret, var_interet, kernel=True):
        '''
        s : number of the iteration (coalition)
        combination_list_sampled : combination of features in which to compute the marginal effect on the AUC
        p : number of features
        ...
        Metric_ind_vinteret : Individual AUC computed with the variable of interest for each combination of feature (filled as it goes along the loop)
        var_interet : number of the variable of interest
        kernel : flag indicating whether to use kernel method (default=True)
        '''
        import math
        import pandas as pd
