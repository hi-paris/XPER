...
for var in np.arange(10): # loop on the number of variables
    ...
    Contrib_AUC = EM.AUC_PC_pickle(y = y_test.iloc[:200].values,          # Target values
                                   X = X_test_dummies.iloc[:200].values,  # Feature values
...
df_phi_i_j_AUC = pd.DataFrame(index=np.arange(len(y_test.iloc[:200])),columns=np.arange(X_test_dummies.iloc[:200].shape[1]))
...
# Careful: Only works if we launch the code with a lot of coalitions, otherwise
# we do not have any values for the performance metric and the benchmark.
...
AUC = roc_auc_score(y_test.iloc[:200], y_hat_proba[:200])  # Compute the AUC on the test sample   

Eval_Metric = ["AUC"] # Name of the chosen metricgest



N_coalition_sampled = 1022   # 2**10 - 2


start_time = datetime.now()

    
Contrib_AUC_Kernel = EM.AUC_Kernel(y = y_test.iloc[:200].values,          # Target values
                                   X = X_test_dummies.iloc[:200].values,  # Feature values
                                   Pred_Formula = model,       # Estimated model
                                   Eval_Metric = Eval_Metric,  # Name of the performance metric
                                   N_coalition_sampled = N_coalition_sampled) # Number of coalitions taken into account for XPER computation
...
variable_name = ["Job tenure","Age", 
           "Car price", "Funding amount",
           'Down payment', 'Loan duration', 'Monthly payment',
           'Credit event', 'Married',
           'Homeowner'] 
    # Give a name to the features
...
X_df = pd.DataFrame(X_test_dummies)
...
shap_values.data = X_test_dummies           # Data/Inputs
...
X_ind_i = pd.Series(np.round(X_test_dummies[ind_i,:],3),index=variable_name).astype(object)
