...
def AUC_Kernel(y, X, Pred_Formula, Eval_Metric, N_coalition_sampled = 1000, seed = 42):
...
#    Code below specific to the exact computation of the XPER values 
...
def XPER(y, X, Pred_Formula, Eval_Metric, var_interet=None, N_coalition_sampled = 1000,
         seed = 42, kernel=True): # Add the kernel parameter
    ...
    if Eval_Metric == ["AUC"]: 
         ...
         with concurrent.futures.ThreadPoolExecutor() as executor:
             ...
             if kernel == True:
                 ...
                 results = [executor.submit(Optimisation.loop_AUC,s,combination_list_sampled,p,X,y,Pred_Formula,delta_n3,Metric,Metric_ind,N,X_shuffle,Metric_vinteret=None,Metric_ind_vinteret=None,var_interet=None,kernel=True) for s in list(range(N_coalition_sampled))]
                
             else:
                 
                 results = [executor.submit(Optimisation.loop_AUC,s,combination_list_sampled,p,X,y,Pred_Formula,delta_n3,Metric,Metric_ind,N,X_shuffle,Metric_vinteret,Metric_ind_vinteret,var_interet,kernel=False) for s in list(range(N_coalition_sampled))]
